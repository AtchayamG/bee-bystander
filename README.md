...
> **Privacy**: [PRIVACY.md](./PRIVACY.md) — what it stores, what leaves the machine, and how withdrawal erases a person  
...
| **Cross-Origin & Network Guard (D10)** | Only the Bystander surface's own origin may call the server from a browser; foreign origins get **403** on every route including `/mcp` (the MCP Streamable HTTP spec requires Origin validation); a rebound `Host` gets 403; the service binds **127.0.0.1**, not every interface | `services/bystander/tests/origin-guard.test.ts`, `services/bystander/tests/entrypoint-bind.test.ts`, `netstat -ano \| findstr :3002` | **VERIFIED 2026-09-23, and it was a real hole until then.** The server answered every origin with `Access-Control-Allow-Origin: *`, so any web page open in the wearer's browser could `PATCH` a bystander from REVOKED to CONSENTED — the new test was run against the old code first and that write returned **200**. It also listened on `0.0.0.0` while its log line said 127.0.0.1. Both fixed; requests with no Origin (curl, Node MCP clients) are still served, because they are already on the machine |
| **Withdrawal Erases the Person (D11)** | Un-enrolling someone removes their name from the database file, its WAL and SHM, the ledger API and the event feed | `services/bystander/tests/withdrawal-erasure.test.ts` (byte-level search of every file, with a control that finds a name that is really there) | **VERIFIED 2026-09-23, and it was a real defect until then.** Withdrawal deleted the ledger row and then wrote the person's name into the append-only events log (`Un-enrolled participant "…"`), which no sweep touches — the act that means *forget me* is what made the name permanent. Events now carry the cluster id only, and the database runs with `secure_delete` on: switching it off makes the test fail with the name still readable on disk, so it is load-bearing |
